...
<c:if test="${furnitures.size() == 0}">
  <h3 style="margin: 50px">No posts available!</h3>
</c:if>
<c:if test="${furnitures.size() != 0}">
  <table class="table">
    <thead>
    ...
    <c:forEach items="${furnitures}" var="Furniture">
      <tr>
        ...
        <td>
          <form action="${pageContext.request.contextPath}/furniture/order/${Furniture.id}" method="post">
            <input type="hidden" name="user_id" value="${user.id}">
            <button>Order</button>
          </form>
        </td>
...
<form action="${pageContext.request.contextPath}/furniture/myOrders/${user.id}" method="post">
  <button class="submit">My orders</button>
</form>
</body>
</html>

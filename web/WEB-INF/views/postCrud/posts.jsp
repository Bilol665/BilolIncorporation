...
<div class="admin-side">
  <div class="addition">
<form action="${pageContext.request.contextPath}/furniture/add/${user.id}" method="post" id="input-form">
  ...
  <textarea name="baseMaterial" placeholder="Enter the materials(wood,steel)(if you don't have this just write 'None')" cols="60" rows="5" required></textarea>
  <textarea name="material" placeholder="Enter the material(leather,cotton)(if you don't have this just write 'None')" cols="60" rows="5" required></textarea>
  <textarea name="colors" placeholder="Colors(if you don't have this just write 'None')" cols="60" rows="5" required></textarea>
  <textarea name="description" placeholder="Add description to your post" cols="60" rows="5" required></textarea>
  ...
  <table class="table">
    ...
    <c:forEach items="${furnitures}" var="Furniture">
      <tr>
        ...
        <td>
        <form action="${pageContext.request.contextPath}/furniture/delete/${Furniture.id}" method="post">
          <input type="hidden" name="user_id" value="${user.id}">
          <button>Delete</button>
        </form>
        </td>
        <td>
          <form action="${pageContext.request.contextPath}/furniture/update/${Furniture.id}">
            <input type="hidden" name="user_id" value="${user.id}">
...
<script>
  function showPopupMenu(e) {
    ...
    // Get the JPopupMenu from the server
    let popupMenu = '<%= popupMenuHandler.getPopupMenu() %>';

    // Display the JPopupMenu at the mouse click position
    popupMenu.show(x, y);
  }
</script>--%>
</body>
